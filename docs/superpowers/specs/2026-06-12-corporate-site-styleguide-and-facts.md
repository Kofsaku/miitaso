...
- Ruby/JavaScript/AWS で Salesforce API連携・スクレイピング基盤を要件定義から運用まで単独構築。自社開発のマーケティングツールで掲載データを拡充し、サイトの検索順位を業界8位→2位に引き上げた（2026-07-10訂正: 「車両掲載台数の順位」ではなく検索順位＝SEOの話。掲載台数増→サイト権威向上→検索順位上昇という因果。サイト表記は「誰にでもわかる抽象化」でSEO/検索順位と書く）
...
2. 検索順位 **業界8位 → 2位**（運送・物流SaaS。自社開発マーケティングツールでSEO改善。2026-07-10訂正: 旧「車両掲載台数の業界順位」表記は不正確）
...
（2026-07-10掲載終了: マイクラ English＝動線ごと削除、物件管理システム＝掲載取り下げ。事実として実在はする）

### 匿名事例（クライアントワーク。この粒度・匿名のまま使う）

- 自治体の電子通貨アプリ開発 — ITコンサルタントとして要件定義を担当
- B2B繊維商社のECサイト刷新 — Shopify開発PM（改修・新機能開発）
- 運送・物流SaaS — Salesforce API連携/スクレイピング基盤を単独構築（→自社開発マーケティングツールで検索順位を業界8位→2位に）
